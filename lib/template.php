...
class template
{
	...
	*/
	function set_template()
	{
		...
		if (file_exists($root_path . 'styles/' . $user->theme['template_path'] . '/template'))
		{
			$this->root = $root_path . 'styles/' . $user->theme['template_path'] . '/template';
			$this->cachepath = $root_path . 'cache/tpl_' . str_replace('_', '-', $user->theme['template_path']) . '_';
			...
			{
				$this->inherit_root = $root_path . 'styles/' . $user->theme['template_inherit_path'] . '/template';
			}
		}
		else
		{
	...
	*/
	...
	{
		global $user, $site_hook;

		if (!empty($site_hook) && $site_hook->call_hook(array(__CLASS__, __FUNCTION__), $handle, $include_once, $this))
		{
			if ($site_hook->hook_return(array(__CLASS__, __FUNCTION__)))
			{
				return $site_hook->hook_return_result(array(__CLASS__, __FUNCTION__));
			}
		}
	...
	*/
	function _tpl_load(&$handle)
	{
		...
		{
			include($root_path . 'lib/compile.' . $phpEx);
		}
