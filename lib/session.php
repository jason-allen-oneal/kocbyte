...
class session
{
	...
	*/
	function extract_current_page($root_path)
	{
		...
		// current directory within the site root (for example: adm)
		$root_dirs = explode('/', str_replace('\\', '/', site_realpath($root_path)));
		$page_dirs = explode('/', str_replace('\\', '/', site_realpath('./')));
		...
		// Current page from site root (for example: adm/index.php?i=10&b=2)
		$page = (($page_dir) ? $page_dir . '/' : '') . $page_name . (($query_string) ? "?$query_string" : '');

		// The script path from the webroot to the current directory (for example: /adm/) : always prefixed with / and ends in /
		$script_path = trim(str_replace('\\', '/', dirname($script_name)));

		// The script path from the webroot to the site root (for example: /)
		$script_dirs = explode('/', $script_path);
		array_splice($script_dirs, -sizeof($page_dirs));
		$root_script_path = implode('/', $script_dirs) . (sizeof($root_dirs) ? '/' . implode('/', $root_dirs) : '');

		// We are on the base level (site root == webroot), lets adjust the variables a bit...
		if (!$root_script_path)
		{
	...
	*/
	...
	{
		global $phpEx, $SID, $_SID, $_EXTRA_URL, $db, $config, $root_path;
...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser && $s_forwarded_for === $u_forwarded_for && $referer_valid)
				{
					...
					include_once($root_path . 'lib/auth/auth_' . $method . '.' . $phpEx);
	...
	*/
	...
	{
		global $SID, $_SID, $db, $config, $cache, $root_path, $phpEx;
		...
		// Do we allow autologin on this site? No? Then override anything
...
//				$db->sql_return_on_error(false);
		...
		// We are not able to simply have an empty session_page btw, therefore we need to tell site how to detect this special case.
	...
	*/
	...
	{
		global $SID, $_SID, $db, $config, $root_path, $phpEx;
	...
	*/
	function session_gc()
	{
		global $db, $config, $root_path, $phpEx;
	...
	*
	* @param string $name		Name of the cookie, will be automatically prefixed with the site cookie name. track becomes [cookie_name]_track then.
...
//				$this->session_create(ANONYMOUS);
			...
			// We show a login box here to allow founders accessing the site if banned by IP
			...
			$message = ($ban_row['ban_end']) ? 'SITE_BAN_TIME' : 'SITE_BAN_PERM';

			$message = sprintf($this->lang[$message], $till_date, '<a href="mailto:' . $config['site_contact'] . '">', '</a>');
			$message .= ($ban_row['ban_give_reason']) ? '<br /><br />' . sprintf($this->lang['SITE_BAN_REASON'], $ban_row['ban_give_reason']) : '';
	...
	*/
	function check_dnsbl($mode, $ip = false)
	{
		...
		if ($ip)
		{
			...
			{
				if (site_checkdnsrr($reverse_ip . '.' . $dnsbl . '.', 'A') === true)
				{
	...
	* @return true if uri is on blacklist, else false. Only blacklist is checked (~zero FP), no grey lists
	function check_uribl($uri)
	{
		...
		if ($uri)
		{
			...
			if (site_checkdnsrr($uri . '.multi.uribl.com.', 'A') === true)
			{
				return true;
			}
		}
...
class user extends session
{
	...
	*/
	function user()
	{
		...
		$this->lang_path = $root_path . 'language/';
	}

	/**
	* Function to set custom language path (able to use directory outside of site)
	*
	...
	*/
	...
	{
		global $db, $template, $config, $auth, $phpEx, $root_path, $cache;
		...
		else
		{
			...
			$this->timezone = $config['site_timezone'] * 3600;
			$this->dst = $config['site_dst'] * 3600;
		...
		if (!$this->theme['theme_storedb'] && $this->theme['parse_css_file'])
		{
			...
			$stylesheet = file_get_contents("{$root_path}styles/{$this->theme['theme_path']}/theme/stylesheet.css");
			// Match CSS imports
			$matches = array();
			...
			if (sizeof($matches))
			{
				...
				{
					if ($content = @file_get_contents("{$root_path}styles/{$this->theme['theme_path']}/theme/" . $matches[1][$idx]))
					{
		...
		$this->img_lang = (file_exists($root_path . 'styles/' . $this->theme['imageset_path'] . '/imageset/' . $this->lang_name)) ? $this->lang_name : $config['default_lang'];
		...
		if (!$localised_images)
		{
			...
			if (@file_exists("{$root_path}styles/{$this->theme['imageset_path']}/imageset/{$this->img_lang}/imageset.cfg"))
			{
				$cfg_data_imageset_data = parse_cfg_file("{$root_path}styles/{$this->theme['imageset_path']}/imageset/{$this->img_lang}/imageset.cfg");
		...
		// Call site_user_session_handler() in case external application want to "bend" some variables or replace classes...
		// After calling it we continue script execution...
		site_user_session_handler();
		...
		// Disable site if the install/ directory is still present
		// For the brave development army we do not care about this, else we need to comment out this everytime we develop locally
		if (!defined('DEBUG_EXTRA') && !defined('ADMIN_START') && !defined('IN_INSTALL') && !defined('IN_LOGIN') && file_exists($root_path . 'install') && !is_file($root_path . 'install'))
		{
			...
			{
				$message = (!empty($config['site_disable_msg'])) ? $config['site_disable_msg'] : 'SITE_DISABLE';
			}
			trigger_error($message);
		}

		// Is site disabled and user not an admin or moderator?
		if ($config['site_disable'] && !defined('IN_LOGIN') && !$auth->acl_gets('a_', 'm_') && !$auth->acl_getf_global('m_'))
		{
			if ($this->data['is_bot'])
			{
				send_status_line(503, 'Service Unavailable');
		...
		if ($config['limit_load'] && $this->load !== false)
		{
			...
			{
				// Set site disabled to true to let the admins/mods get the proper notification
				$config['site_disable'] = '1';
		...
		if (!defined('IN_ADMIN') && !defined('ADMIN_START') && $config['chg_passforce'] && !empty($this->data['is_registered']) && $auth->acl_get('u_chgpasswd') && $this->data['user_passchg'] < time() - ($config['chg_passforce'] * 86400))
		{
			...
			{
				redirect(append_sid("{$root_path}ucp.$phpEx", 'i=profile&amp;mode=reg_details'));
			}
		}
	...
	* Function to mimic sprintf() with the possibility of using site's language system to substitute nullar/singular/plural forms.
	...
	function set_lang(&$lang, &$help, $lang_file, $use_db = false, $use_help = false)
	{
		...
		if (!$use_db)
		{
			...
			if (!file_exists($language_filename))
			{
				...
				{
					// The user's selected language is missing the file, the site default's language is missing the file, and the file doesn't exist in /en.
	...
	*/
	function img($img, $alt = '', $width = false, $suffix = '', $type = 'full_tag')
	{
		...
		if (empty($img_data))
		{
			...
			// Use URL if told so
			$root_path = (defined('SITE_USE_SITE_URL_PATH') && SITE_USE_SITE_URL_PATH) ? generate_site_url() . '/' : $root_path;
			...
			// We overwrite the width and height to the site logo's width
			// and height here if the contents of the site_logo file are
			// really equal to the site_logo
			// This allows us to change the dimensions of the site_logo without
			// modifying the imageset.cfg and causing a conflict for everyone
			// who modified it for their custom logo on updating
			if ($img == 'site_logo' && file_exists($root_path . $path))
			{
				global $cache;
				...
				{
					$img_file_hashes[$key] = md5(file_get_contents($root_path . $path));
					$cache->put('imageset_site_logo_md5', $img_file_hashes);
				}

				$site_logo_hash = '0c461a32cd3621643105f0d02a772c10';

				if ($site_logo_hash == $img_file_hashes[$key])
				{
					$img_data['width'] = '149';
					$img_data['height'] = '52';
				}
	...
	*/
	function optionget($key, $data = false)
	{
		...
		return site_optionget($this->keyoptions[$key], $var);
	}
	...
	*/
	function optionset($key, $value, $data = false)
	{
		...
		$new_var = site_optionset($this->keyoptions[$key], $value, $var);
	...
	*/
	function leave_newly_registered()
	{
		...
		if (!function_exists('remove_newly_registered'))
		{
			...
			include($root_path . 'lib/user.' . $phpEx);
		}
