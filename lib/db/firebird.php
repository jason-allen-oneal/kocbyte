...
require_once $phpbb_root_path.'includes/classes/db/dbal.'.$phpEx;
...
class dbal_firebird extends dbal{
	...
	public function sql_connect($sqlserver, $sqluser, $sqlpassword, $database, $port = false, $persistency = false){
		...
		if($this->persistency){
			if(!function_exists('ibase_pconnect')){
				$this->connect_error = 'ibase_pconnect public function does not exist, is interbase extension installed?';
				return $this->sql_error('');
			}
			$this->db_connect_id = @ibase_pconnect($use_database, $this->user, $sqlpassword, false, false, 3);
		}else{
			if(!function_exists('ibase_connect')){
				$this->connect_error = 'ibase_connect public function does not exist, is interbase extension installed?';
				return $this->sql_error('');
			}
		...
		if($query != ''){
			...
			if($this->query_result === false){
				...
				if(strlen($query) > 32767){
					...
					}elseif(preg_match('/^(UPDATE ([\\w_]++)\\s+SET )([\\w_]++\\s*=\\s*(?:\'(?:[^\']++|\'\')*+\'|\\d+)(?:,\\s*[\\w_]++\\s*=\\s*(?:\'(?:[^\']++|\'\')*+\'|[\d-.]+))*+)\\s+(WHERE.*)$/s', $query, $data)){
				...
				}elseif(($this->query_result = @ibase_query($this->db_connect_id, $query)) === false){
					$this->sql_error($query);
				}
	...
	* Build LIMIT query
	*/
	public function _sql_query_limit($query, $total, $offset = 0, $cache_ttl = 0){
	...
	*/
	...
	public function _sql_bit_and($column_name, $bit, $compare = ''){
		return 'BIN_AND('.$column_name.', '.(1 << $bit).')'.(($compare) ? ' '.$compare : '');
	}

	public function _sql_bit_or($column_name, $bit, $compare = ''){
