...
class acm
{
	...
	*/
	function save()
	{
		...
		{
			if (!function_exists('site_is_writable'))
			{
				global $root_path;
				include($root_path . 'lib/functions.' . $phpEx);
			}

			// Now, this occurred how often? ... phew, just tell the user then...
			if (!site_is_writable($this->cache_dir))
			{
	...
	*/
	function _write($filename, $data = null, $expires = 0, $query = '')
	{
		...
		if ($handle = @fopen($file, 'wb'))
		{
			...
			if (!function_exists('site_chmod'))
			{
				global $root_path;
				include($root_path . 'lib/functions.' . $phpEx);
			}

			site_chmod($file, CHMOD_READ | CHMOD_WRITE);
	...
	*/
	function remove_file($filename, $check = false)
	{
		...
		if ($check && !site_is_writable($this->cache_dir))
		{
